...
//    prefix sum modulo; time: O(n), space: O(n)
    public static int minSubarray(int[] nums, int p) {
        int n = nums.length;
        int totalSum = 0;
//        Step 1: calculate the totalSum and the target remainder
        for(int num : nums)
            totalSum = (totalSum + num) % p;
        int target = totalSum % p;
//        if already divisible, no sub array needs to be removed
        if(target == 0) return 0;

//        otherwise, use a map to track prefix sum mod p
